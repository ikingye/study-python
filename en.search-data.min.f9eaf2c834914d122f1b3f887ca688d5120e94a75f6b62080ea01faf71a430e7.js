"use strict";(function(){const t={cache:!0};t.doc={id:"id",field:["title","content"],store:["title","href","section"]};const e=FlexSearch.create("balance",t);window.bookSearchIndex=e,e.add({id:0,href:"/study-python/docs/basic/",title:"第一部分 基础入门",section:"Docs",content:" Python 工具最佳实践 # pipx + pyenv + poetry\npipx: python 命令行应用管理，类似 brew/macos，apt/ubuntu，yum/centos pypa/pipx pyenv: python 版本管理 pyenv/pyenv poetry: python 项目管理/虚拟环境管理 python-poetry/poetry 安装 # MacOS # # 先安装 pipx brew update brew install pipx # 再安装 poetry pipx install poetry # 安装 pyenv brew install pyenv # 从镜像下载 cat &lt;&lt; &#39;EOF&#39; &gt;&gt; ~/.zshrc function pyenvinstall { wget -c https://npm.taobao.org/mirrors/python/$@/Python-$@.tar.xz -P ~/.pyenv/cache/ pyenv install $@ } EOF zsh # 更新 pyenv 缓存 pyenv update # 列出可选的版本 pyenv install -l # 安装需要的 python 版本 pyenvinstall 3.12.3 CentOS # # 安装 pipx # sudo yum install -y python3 python3 -m pip install --user pipx python3 -m pipx ensurepath # 升级 python3 -m pip install --user --upgrade pipx # 安装 pyenv curl https://pyenv.run | bash 使用 # "}),e.add({id:1,href:"/study-python/docs/basic/other/package/manage/",title:"包管理",section:"Package",content:" Python 包管理 # pip # 加速 # mkdir -p ~/.config/pip cat &lt;&lt; EOF &gt; ~/.config/pip/pip.conf [global] index-url = https://pypi.tuna.tsinghua.edu.cn/simple extra-index-url = https://mirrors.aliyun.com/pypi/simple https://pypi.doubanio.com/simple https://mirrors.tuna.tsinghua.edu.cn EOF "}),e.add({id:2,href:"/study-python/docs/basic/other/version/manage/",title:"版本管理",section:"版本",content:" Python 版本管理 # pyenv # pyenv 加速 # 在 .zshrc 添加\nfunction pyenvinstall { wget -c https://npm.taobao.org/mirrors/python/$@/Python-$@.tar.xz -P ~/.pyenv/cache/ pyenv install $@ } 执行 zsh 使配置生效， 之后即可这样安装： pyenvinstall 3.8.2\npyenv 基础命令 # # List all available versions pyenv install -l # 更新 pyenv update brew update &amp;&amp; brew upgrade pyenv pyenv update # pyenv/pyenv-update git clone https://github.com/pyenv/pyenv-update.git $(pyenv root)/plugins/pyenv-update pyenv update pyenv global 3.8.2 2.7.17 # set by ~/.pyenv/version\n3.8.2 2.7.17 pyenv shell 3.8.2 2.7.17 # set by PYENV_VERSION environment variable\n$ echo $PYENV_VERSION 3.8.2:2.7.17 pyenv 优先级 # pyenv shell $PYENV_VERSION pyenv local .python-version file pyenv global ~/.pyenv/version System Python "}),e.add({id:3,href:"/study-python/docs/basic/grammar/",title:"1.1 语法",section:"第一部分 基础入门",content:` 语法 # range vs xrange # range 是直接返回一个保存全量数据的 list, 空间复杂度是 O(n),
...
参考：
What is tail recursion? What is tail call optimization? 参考 # Python 语言参考 Python 标准库 `}),e.add({id:4,href:"/study-python/docs/advanced/pattern/",title:"2.1 设计模式",section:"第二部分 进阶实战",content:" Python 设计模式 # 参考 # faif/python-patterns # "}),e.add({id:5,href:"/study-python/docs/code/type/",title:"3.1 数据类型",section:"第三部分 源码实现",content:" Python 数据类型 # "}),e.add({id:6,href:"/study-python/docs/appendix/tutorial/",title:"4.1 教程",section:"第四部分 附录",content:` 教程 # 基础 # satwikkansal/wtfpython # leisurelicht/wtfpython-cn 一些有趣且鲜为人知的 Python 特性
...
`}),e.add({id:7,href:"/study-python/docs/basic/other/package/common/",title:"常用",section:"Package",content:` 常用 Package # 网络 # twisted/twisted # Event-driven networking engine written in Python. https://www.twistedmatrix.com
Twisted Introduction
This multi-part series introduces Asynchronous Programming and the Twisted networking framework.
`}),e.add({id:8,href:"/study-python/docs/basic/std/",title:"1.2 标准库",section:"第一部分 基础入门",content:" Python 标准库 # "}),e.add({id:9,href:"/study-python/docs/advanced/algs/",title:"2.2 算法实现",section:"第二部分 进阶实战",content:" Python 算法实现 # "}),e.add({id:10,href:"/study-python/docs/code/keyword/",title:"3.2 关键字",section:"第三部分 源码实现",content:" Python 关键字 # "}),e.add({id:11,href:"/study-python/docs/appendix/interview/",title:"4.2 面试题",section:"第四部分 附录",content:` 面试题 # 参考：
...
Python 猴子补丁 python 面试题精讲 ——monkey patch（猴子补丁） `}),e.add({id:12,href:"/study-python/docs/basic/grammar/builtin/",title:"内建函数",section:"1.1 语法",content:` 内建函数 # all # 判断给定的可迭代参数 iterable 中的所有元素是否都为 TRUE
...
`}),e.add({id:13,href:"/study-python/docs/basic/grammar/function/",title:"函数",section:"1.1 语法",content:` 函数 # __init__ vs __new__ # __new__ 最先被调用，通常用于控制生成一个新实例的过程。 它是类级别的方法。
...
Python 协程 `}),e.add({id:16,href:"/study-python/docs/basic/grammar/dict/",title:"字典",section:"1.1 语法",content:` 字典 # 遍历 # for key in d: # For Python 3.x: for key, value in d.items(): # For Python 2.x: for key, value in d.iteritems(): items # The operation items() will work for both 2 and 3, but in 2 it will return a list of the dictionary&rsquo;s (key, value) pairs, which will not reflect changes to the dict that happen after the items() call.
...
参考：
In Python 2, what are the performance trade-offs for viewvalues()/viewitems() over itervalues()/iteritems()? `}),e.add({id:17,href:"/study-python/docs/basic/other/version/",title:"版本",section:"1.4 其他",content:` Python 版本 # 3 # 3.9 # 3.9.0 (2020-10-05) # 3.8 # 3.8.6 (2020-09-24) # 3.8.5 (2020-07-20) # 3.8.0 (2019-10-14) # 3.7 # 3.7.9 (2020-08-17) # 3.7.0 (2018-06-27) # 2 # 2.7 # 2020-01-01 已经废弃
2.7.18 (2020-04-20) # 2.7.0 # 2.6 # 2.6.9 (2013-10-29) # 2.6.0 (2008-10-02) # `}),e.add({id:18,href:"/study-python/docs/basic/grammar/generator/",title:"生成器",section:"1.1 语法",content:` 生成器 # iterable 可迭代的 # Everything you can use &ldquo;for&hellip; in&hellip;&rdquo; on is an iterable; lists, strings, files&hellip;
iterable 包括：
...
python 中 yield 的用法详解 —— 最简单，最清晰的解释 What does the “yield” keyword do? python generator “send” function purpose? `}),e.add({id:19,href:"/study-python/docs/basic/grammar/class/",title:"类",section:"1.1 语法",content:` 类 # 问答 # class 后面加不加 object 的区别 # python 3 中已经默认就帮你加载了 object 了（即便你没有写上 object）
...
参考：
理解 Python 中的 init_subclass `}),e.add({id:20,href:"/study-python/docs/basic/grammar/function/decorator/",title:"装饰器",section:"函数",content:` 装饰器 # 函数参数能够接收别的函数，就是高阶函数
decorator 就是一个返回函数的高阶函数
import functools def log(text): def decorator(func): @functools.wraps(func) def wrapper(*argc, **kw): print(&#39;%s %s()&#39; % (text, func.__name)) return func(*args, **kw) return wrapper return decorator @log(&#39;execute&#39;) def now(): print(&#39;2020-04-26&#39;) `}),e.add({id:21,href:"/study-python/docs/basic/spec/",title:"1.3 编程规范",section:"第一部分 基础入门",content:" 编程规范 # Google Python Style Guide Python 风格指南 "}),e.add({id:22,href:"/study-python/docs/advanced/pkg/",title:"2.3 常用库",section:"第二部分 进阶实战",content:` Python 常用库 # log # rochacbruno/dynaconf # Configuration Management for Python https://dynaconf.com/
`}),e.add({id:23,href:"/study-python/docs/code/runtime/",title:"3.3 运行时",section:"第三部分 源码实现",content:" Python 运行时 # "}),e.add({id:24,href:"/study-python/docs/appendix/attention/",title:"4.3 关注项目",section:"第四部分 附录",content:` 关注项目 # Web # pallets/flask # The Python micro framework for building web applications.
...
`}),e.add({id:25,href:"/study-python/docs/basic/other/",title:"1.4 其他",section:"第一部分 基础入门",content:" 其他 # "}),e.add({id:26,href:"/study-python/docs/advanced/snippet/",title:"2.4 代码片段",section:"第二部分 进阶实战",content:" Python 代码片段 # "}),e.add({id:27,href:"/study-python/docs/advanced/test/",title:"2.5 测试",section:"第二部分 进阶实战",content:" Python 测试 # "}),e.add({id:28,href:"/study-python/docs/basic/other/version/3.7/",title:"3.7",section:"版本",content:` Python 3.7 # 安装 # 依赖 # sudo yum install readline readline-devel readline-static -y sudo yum install openssl openssl-devel openssl-static -y sudo yum install sqlite-devel -y sudo yum install bzip2-devel bzip2-libs -y libffi-devel # 3.7 版本需要一个新的包 libffi-devel，安装此包之后再次进行编译安装即可。
...
`}),e.add({id:29,href:"/study-python/docs/basic/other/package/",title:"Package",section:"1.4 其他",content:" Package # "}),e.add({id:30,href:"/study-python/docs/advanced/prof/",title:"2.6 性能",section:"第二部分 进阶实战",content:" Python 性能 # cProfile # VizTracer # gaogaotiantian/viztracer "}),e.add({id:31,href:"/study-python/docs/basic/other/anaconda/",title:"Anaconda",section:"1.4 其他",content:` Anaconda # Anaconda 是一个免费开源 的 Python 和 R 语言的发行版本，用于计算科学（数据科学、机器学习、大数据处理和预测分析），Anaconda 致力于简化包管理和部署。
Anaconda 的包使用软件包管理系统 Conda 进行管理。
`}),e.add({id:32,href:"/study-python/docs/advanced/frame/",title:"2.7 框架",section:"第二部分 进阶实战",content:" Python 框架 # "})})()